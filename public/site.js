...
/* forms — real delivery via Web3Forms (access key is public/client-side by design) */
var W3_KEY='4f93c894-d8be-4f04-b4a7-fa68fed46615';
function w3send(form,subject){var fd=new FormData(form);fd.append('access_key',W3_KEY);fd.append('from_name','SimpliiGood Website');if(subject)fd.append('subject',subject);return fetch('https://api.web3forms.com/submit',{method:'POST',headers:{'Accept':'application/json'},body:fd}).then(function(r){return r.json();});}
document.querySelectorAll('form[data-confirm]').forEach(function(f){f.addEventListener('submit',function(e){e.preventDefault();if(!f.checkValidity()){f.reportValidity();return;}var b=f.querySelector('button[type=submit]');var ok=function(){var m=f.querySelector('.formmsg');if(m)m.style.display='block';if(b)b.textContent=f.dataset.confirm||'Sent';f.querySelectorAll('input,select,textarea,button').forEach(function(x){x.disabled=true;});};var err=function(){if(b){b.textContent='Try again';b.disabled=false;}var em=f.querySelector('.formerr');if(em)em.style.display='block';};if(b){b.disabled=true;b.textContent='Sending…';}w3send(f,f.dataset.subject||'New enquiry — SimpliiGood').then(function(d){if(d&&d.success){ok();}else{err();}}).catch(err);});});
(function(){var nf=document.getElementById('newsForm');if(!nf)return;nf.addEventListener('submit',function(e){e.preventDefault();if(!nf.checkValidity()){nf.reportValidity();return;}var b=nf.querySelector('button');var i=nf.querySelector('input[type=email]');if(b){b.disabled=true;b.textContent='…';}var done=function(){if(b)b.textContent="You're in";if(i){i.value='';i.disabled=true;}};w3send(nf,'Newsletter signup — SimpliiGood').then(done).catch(done);});})();
